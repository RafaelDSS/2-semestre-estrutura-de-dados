...
Node *node_at(List *list, int position) {
    ...
    while (qt_positions < position-1 && aux_node->next != list->head) {
        aux_node = aux_node->next;
        qt_positions++;
    }
...
int push_sorted(List *list, Aluno aluno) {
    ...
    if (list_is_empty(list) || strcmp(list->head->aluno.matricula, aluno.matricula) >= 0)
        ...
        while (aux_node->next != list->head && strcmp(aux_node->aluno.matricula, aluno.matricula) < 0)
            aux_node = aux_node->next;
...
void update_pointer_tail(List *list) {
    Node *aux_node;

    if (!list_is_empty(list)) {
        aux_node = node_back(list);
        aux_node->next = list->head;
    }
}
